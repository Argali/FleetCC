...
router.get("/users", requireAuth, requireRole("fleet_manager"), (req, res) => {
  const users = usersStore.getAllUsers().map(u => ({
    id: u.id, name: u.name, email: u.email, role: u.role, tenant_id: u.tenant_id, active: u.active,
  }));
...
router.post("/users", requireAuth, requireRole("fleet_manager"), (req, res) => {
  ...
  res.status(201).json({ ok: true, data: { id: newUser.id, name: newUser.name, email: newUser.email, role: newUser.role, active: newUser.active } });
});
